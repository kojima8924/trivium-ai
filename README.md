...
- **3 系統の課題** — **READ** 要旨・推論・批判的読解 / **WRITE** 構成・明確さ・根拠・推敲 / **LOGIC** 短い Python の読解と、手順・条件・推論のパズル（内部キーは `CODE`）。手書きの課題 **63 問**（単独 51 問 + 2〜3 系統にまたがる複合課題 12 問）に加えて、生成・検証済みのストック **236 問**（READ 70 / WRITE 70 / LOGIC 70 / 複合 26。問題タイプ 15 種＋複合）。難易度は系統ごとに **1（誰でも解ける）〜10（非常に難しい）**、各課題は難易度ベクトル `{ read, write, code }` を持つ
...
- **教材の推薦（ADVISOR と会話）** — 「おすすめの本は？」「LOGIC を伸ばす教材」と聞くと、到達レベル・弱い小分類・直近の失敗から、定番書・公式サイト・演習サイトのカタログ（`src/lib/materials/catalog.ts`、85 件。実在の定番書・公式サイト・演習サイト）と Dify ナレッジ検索（設定時）のスコアで候補を選び、案内役が理由つきで提案
...
手書きの 63 問に加えて、**難易度 1〜10 × 問題タイプ 15 種（READ 5 / WRITE 5 / LOGIC 5）＋ 複合 3 型**の課題を `scripts/stock/gen_stock.mts` で事前生成し、`src/lib/tasks/stock/*.generated.ts` に同梱している（READ 70 / WRITE 70 / LOGIC 70 / 複合 26 = **236 問**。手で編集しない）。難易度の設計は 1＝小学校中学年でも迷わない、5＝高校標準、10＝上級者でも数分の集中と検証が要る。
...
| 5. 推薦 | `RECOMMENDATIONS` | 旧「今日の 1 冊」の固定リスト（互換のため残置）。現在の推薦は教材カタログ `src/lib/materials/catalog.ts`（85 件）と推薦エンジン `src/lib/materials/recommend.ts` が担い、LINE・Dashboard・日次総評で共通。LLM に書名を作らせない |
