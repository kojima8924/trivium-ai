...
- **教材の推薦（ADVISOR と会話）** — 「おすすめの本は？」「LOGIC を伸ばす教材」と聞くと、到達レベル・弱い小分類・直近の失敗から、定番書・公式サイト・演習サイトのカタログ（`src/lib/materials/catalog.ts`、60 件超）と Dify ナレッジ検索（設定時）のスコアで候補を選び、案内役が理由つきで提案
