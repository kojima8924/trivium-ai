...
Python ゲート: 難易度 1〜3 では Python 読解・バグ発見を出さない（Python 系で正解済み、または本人がそのタイプを指定したときは解禁）。
              文法の未知を「論理の失敗」として否定証拠にしないための措置（pythonGateAllows）
```
...
副作用のある層と、判断のロジックを分けてある。テスト（`tests/*.test.ts`、226 件）はほぼすべて後者を直接呼ぶので、DB も API キーも要らずに走る。
