...
/** サイト内の相対パス（`/characters/read.png?v=1` / `/characters/read-face.png?v=1`） */
export function characterImagePath(agent: AgentKey, variant: CharacterVariant = "full"): string {
  const file = CHARACTER_META[agent].file;
  const name = variant === "face" ? file.replace(/\.png$/, "-face.png") : file;
 ...
 */
export function characterImageUrl(agent: AgentKey, appUrl: string, variant: CharacterVariant = "face"): string {
  return `${appUrl.replace(/\/$/, "")}${characterImagePath(agent, variant)}`;
}
