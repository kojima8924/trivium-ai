...
// CODE: 74 問（difficulty 1〜10・問題タイプ付き）。生成: gpt-5.5 / 検証: Python 実行 + 独立ソルバー gpt-5.5 + レビュー gpt-5.4-mini
...
export const CODE_STOCK: Task[] = [
  ...
  },
  {
    ...
    "difficulty": 1,
    "title": "同じ時刻の予定",
    "passage": "ゆうとは、水曜日の4時に公園へ行く予定です。\n同じ水曜日の4時に、家でピアノの練習をする予定もあります。\n公園と家に同時にいることはできません。",
    "prompt": "この予定の矛盾として正しいものはどれですか。",
    "kind": "choice",
    "taskType": "puzzle",
    "hints": [
      "2つの予定は、いつになっていますか。",
      "時刻と場所をそれぞれ比べてみましょう。",
      "同じ時刻にできることかどうかを考えましょう。"
    ],
    "explanation": "2つの予定はどちらも水曜日の4時です。しかし場所は公園と家で、同時にいることはできないため矛盾しています。",
    "skillTags": [
      "tracing"
    ],
    "choices": [
      "ピアノの練習をする場所が公園になっている",
      "水曜日には予定が何も入っていない",
      "同じ時刻に、公園と家の両方にいることになっている",
      "公園へ行く日と、ピアノの練習の日がちがっている"
    ],
    "answerKey": [
      "2"
    ]
  ...
  },
  {
    ...
    "difficulty": 8,
    "title": "方角格子の工房",
    "passage": "ある町の地図は、南北4段・東西4列の正方形の区画に分かれている。北から1段目〜4段目、西から1列目〜4列目と呼ぶ。6つの施設、駅・灯台・市場・図書館・公園・工房は、それぞれ異なる区画に1つずつある。\n\n方角について、「北にある」は段番号が小さいこと、「東にある」は列番号が大きいことを意味する。「隣接」は辺を共有する区画どうしだけを指し、斜めは隣接に含めない。距離は、縦横に何区画移動するかの合計で数える。\n\n条件は次のとおり。\n1. 駅と灯台は対角の角にあり、灯台は駅の北東にある。\n2. 図書館は南端にあり、駅に隣接していない。また、図書館から駅までの距離は、図書館から灯台までの距離より短い。\n3. 公園は東端にあり、灯台に隣接していない。公園は図書館と同じ段にはなく、駅から公園までの距離は駅から図書館までの距離より長い。\n4. 市場は端の区画にはない。市場は「公園の真西にある」と「図書館の真北にある」のうち、ちょうど一方だけを満たす。\n5. 工房は市場に隣接しているが、公園にも図書館にも隣接していない。\n6. 工房から駅までの距離と、工房から灯台までの距離は等しくない。\n7. 市場は工房より南にある。",
    "prompt": "工房がある区画はどこか。",
    "kind": "choice",
    "taskType": "puzzle",
    "hints": [
      "まず、駅と灯台の位置は条件1だけでどこまで決まりますか。",
      "図書館と公園を決めたあと、市場は条件4により2通りに絞れます。",
      "市場の2通りそれぞれで、工房の隣接条件と距離条件、最後の南北条件を順に照合しましょう。"
    ],
    "explanation": "条件1より、駅は南西の角、灯台は北東の角。条件2から図書館は南端のうち、駅に隣接せず駅により近い区画なので、北から4段目・西から3列目。条件3から公園は北から3段目・西から4列目。\n市場は端ではなく、「公園の真西」と「図書館の真北」のちょうど一方なので、候補は北から3段目・西から2列目、または北から2段目・西から3列目。前者だと工房候補は最終的に北から3段目・西から1列目になるが、市場が工房より南ではない。後者なら工房は条件5・6を満たす北から1段目・西から3列目で、市場は工房より南にある。したがって正解は北から1段目・西から3列目。",
    "skillTags": [
      "tracing"
    ],
    "choices": [
  ...
  },
  {
    ...
    "difficulty": 8,
    "title": "周期条件の番号数",
    "passage": "1から10000までの整数 n について、次の3条件をすべて満たすものを「当たり」とする。条件1: 2^n+n^2 を7で割った余りが1である。条件2: 3^n+n を5で割った余りが1である。条件3: n が4の倍数でない、または、5^n を9で割った余りが n+1 を9で割った余りに等しい。",
    "prompt": "当たりとなる n は何個あるか。",
    "kind": "choice",
    "taskType": "math",
    "hints": [
      "それぞれの条件は、n をどの数で割った余りだけを見れば判定できますか。",
      "条件1は n mod 21、条件2は n mod 20 にまとめられます。まずこの2つを同時に満たす余りを考えましょう。",
      "条件3は、条件2の候補のうち n が4の倍数になる場合だけ追加で調べれば十分です。最後に周期全体と端数部分に分けて数えます。"
    ],
    "explanation": "条件1は 2^n の周期3と n^2 の法7から、n≡0,2,5 mod 21。条件2は 3^n の周期4と n の法5から、n≡0,2,13,19 mod 20。これらを合わせると法420で12通り。うち n≡0 mod 20 の3通りだけ条件3の追加確認が必要で、法1260で見ると n≡0 mod 1260 だけが残り、他2通りは消える。したがって法1260で当たりは 1+9×3=28 通り。10000=1260×7+1180 なので、7周期で196個、残り1180までに24個あり、合計220個。",
    "skillTags": [
      "algorithms",
      "tracing"
    ],
  ...
  },
  {
    ...
    "difficulty": 9,
    "title": "六枚の真偽札",
    "passage": "机の上に A、B、C、D、E、F の6枚の札がある。各札は青か白のどちらかである。\n\n青い札に書かれた文は真、白い札に書かれた文は偽である。また、青い札はちょうど3枚である。\n\n各札には次の文が書かれている。\nA：Bが青であることと、Cが青であることは同値である。\nB：Aが青なら、Fは青である。\nC：DとEのうち、少なくとも一方は白である。\nD：AとEは同じ色である。\nE：Bが白なら、Aは青である。\nF：Cが白なら、Dは白である。",
    "prompt": "青い札の組合せとして正しいものはどれか。",
    "kind": "choice",
    "taskType": "puzzle",
    "hints": [
      "それぞれの札について、「青である」と「文が真である」が同じ意味になることを、どのように記号化できますか。",
      "Aの色を仮に決めると、D、E、C、B、Fの色が連鎖的に制限されます。もう一方の仮定も同じように確かめましょう。",
      "矛盾が出る仮定では、ある札が青でないはずなのに、その文の内容から青でなければならなくなります。"
    ],
    "explanation": "Aが青だと仮定する。Aの文が真なのでBとCは同じ色。Dの文とEの文を合わせて追うと、Eは青、Dも青になる。するとCの文「DとEの少なくとも一方は白」は偽なのでCは白。Aの文よりBも白。Bの文「Aが青ならFは青」は、Aが青でFが白のときだけ偽なので、Bが白であることと合う。Fの文「Cが白ならDは白」は、Cが白でDが青なので偽となり、Fは白であることと合う。よって青はA・D・Eの3枚。\n逆にAが白だと、Aの文が偽なのでBとCは異なる色になる。そこからE、D、C、Bを順に追うと、Bは白でなければならない一方、Bの文は真になってしまい矛盾する。したがって青い札はA・D・Eだけである。",
    "skillTags": [
      "tracing"
    ],
    "choices": [
